...
def calculate_impedance(input_signal, output_signal):
    return np.divide(input_signal, output_signal)
...
def main():
    frequencies = [100, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 7500, 8000, 8500, 9000, 9500, 10000, 11000, 12000, 13000, 14000, 15000, 16000, 17000, 18000, 19000, 20000]
    duration = 1.0
    sampling_rate = 44100
    ...
    for frequency in frequencies:
        ...
        impedance = calculate_impedance(signal, recorded_signal)

        print(f"Amplituda FFT wyjścia / FFT wejścia dla częstotliwości {frequency} Hz:")
        print(amplitude_ratio)
        print(f"Impedancja dla częstotliwości {frequency} Hz:")
        print(impedance)

        plot_signal(signal, sampling_rate, f"Teoretyczny sygnał - {frequency} Hz")
        plot_signal(recorded_signal, sampling_rate, f"Nagrany sygnał - {frequency} Hz")
        plot_signal(amplitude_ratio, sampling_rate, f"Amplituda FFT wyjścia / FFT wejścia dla częstotliwości - {frequency} Hz")
        plot_fft(fft_input, sampling_rate, f"FFT wejście - {frequency} Hz")
        plot_fft(fft_output, sampling_rate, f"FFT wyjście - {frequency} Hz")
